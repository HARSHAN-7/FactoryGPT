...
import { ArrowUpRight, Menu, X, LogIn, Play } from 'lucide-react';
    ...
    <>
      <header className="sticky top-0 z-50 w-full border-b border-industrial-800 bg-industrial-950/95 backdrop-blur-xl shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          {/* Action Buttons & Intro Video Play Button */}
          <div className="hidden md:flex items-center gap-3">
            {/* Watch Intro Video Button */}
            <button
              onClick={() => setIntroOpen(true)}
              className="px-3.5 py-2 rounded-full border border-gold-500/40 hover:border-gold-500 bg-gold-600/10 text-gold-500 font-mono text-xs font-bold transition-all flex items-center gap-1.5 hover:scale-105"
            >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-b border-industrial-800 bg-industrial-900 px-4 pt-4 pb-6 space-y-4 shadow-2xl">
            ...
            <div className="pt-4 border-t border-industrial-800 flex flex-col gap-2.5">
              <button
                onClick={() => { setMobileMenuOpen(false); setIntroOpen(true); }}
                className="w-full py-2.5 rounded-full border border-gold-500/40 text-gold-500 font-mono text-xs font-bold flex items-center justify-center gap-1.5"
              >
      ...
      {/* Intro Video Modal with Skip Button */}
