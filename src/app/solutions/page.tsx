...
export default function SolutionsPage() {
  const solutions = [
    ...
    },
    {
      ...
      icon: <Package className="w-6 h-6 text-gold-500" />,
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16 w-full">
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">
            Tailored AI Solutions for Every <span className="text-gold-500">Factory Department</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {solutions.map((s) => (
            <div key={s.id} className="p-8 rounded-2xl bg-industrial-900 border border-industrial-800 space-y-6 hover:border-gold-500/40 transition-colors shadow-xl">
              <div className="flex items-center justify-between">
                <div className="w-12 h-12 rounded-xl bg-gold-600/10 border border-gold-500/30 flex items-center justify-center">
                  {s.icon}
                </div>
                <span className="text-[10px] font-mono text-gold-500 px-3 py-1 rounded-full bg-gold-600/10 border border-gold-500/20 uppercase tracking-wider font-bold">
                  {s.category}
                </span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-industrial-400 leading-relaxed">{s.description}</p>
              </div>

              <div className="space-y-2 pt-4 border-t border-industrial-800">
                <div className="text-xs font-mono text-industrial-500 uppercase font-bold">Key Business Outcomes:</div>
                {s.benefits.map((b) => (
                  <div key={b} className="flex items-center gap-2 text-xs font-mono text-industrial-300">
