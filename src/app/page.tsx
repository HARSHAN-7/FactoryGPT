...
export default function LandingPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      ...
      const res = await signUpWithEmail(email, password, fullName || 'Engineering Operator');
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!res.success) setErrorMsg(res.error || 'Invalid OTP token');
  };

  // If NOT authenticated, show Mandatory Authentication Portal Gate FIRST
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-industrial-950 text-industrial-100 flex flex-col justify-between font-sans bg-grid-pattern selection:bg-gold-600 selection:text-white">
        ...
        <div className="flex-1 flex items-center justify-center p-4 sm:p-8">
          <div className="w-full max-w-md space-y-6">
            ...
            <div className="text-center space-y-2">
              <img src="/logo.png" alt="FactoryGPT Official Logo" className="h-14 mx-auto object-contain" />
              <p className="text-xs font-mono text-gold-500 font-semibold">
                ● MANDATORY INDUSTRIAL OPERATOR AUTHENTICATION GATE
              </p>
              <p className="text-xs text-industrial-400">
                Please log in or create an operator account to access FactoryGPT.
              </p>
            </div>

            {/* Login & Registration Portal Card */}
            <Card className="bg-industrial-900 border-industrial-800 p-6 shadow-2xl space-y-6 gold-glow">
              
              {/* Mode Toggle (Login vs Registration) */}
              <div className="flex items-center justify-center border-b border-industrial-800 pb-4 gap-4 text-xs font-mono">
                <button
                  onClick={() => { setAuthMode('login'); setErrorMsg(null); }}
                  className={`pb-1 font-bold transition-all ${
                    authMode === 'login' ? 'text-gold-500 border-b-2 border-gold-500' : 'text-industrial-400 hover:text-white'
                  }`}
                >
                  LOGIN PORTAL
                </button>
                ...
                <button
                  ...
                  className={`pb-1 font-bold transition-all ${
                    authMode === 'signup' ? 'text-gold-500 border-b-2 border-gold-500' : 'text-industrial-400 hover:text-white'
                  }`}
                >
                  REGISTRATION PORTAL
                </button>
              ...
              {activeTab === 'email' && (
                <form onSubmit={handleEmailAuth} className="space-y-4 text-xs font-mono">
                  ...
                  <Button
                    ...
                    disabled={isLoading}
                    icon={isLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4" />}
                  >
                    {isLoading ? 'Authenticating...' : authMode === 'login' ? 'Unlock FactoryGPT Account' : 'Register Operator Account & Store in DB'}
                  </Button>
                </form>
              )}
              ...
              {activeTab === 'google' && (
                <div className="space-y-4 py-4 text-center">
                  <p className="text-xs text-industrial-400 font-mono">
                    One-click OAuth authentication stored in Supabase profiles database.
                  </p>
                  <button
        ...
        <footer className="py-6 border-t border-industrial-800 text-center text-xs font-mono text-industrial-500">
